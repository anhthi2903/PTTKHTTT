        ...
        private void btnTaoMNCC_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(sCon))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT dbo.f_maNhaCungCap10()", con))
                    {
                        string maNCCMoi = (string)cmd.ExecuteScalar(); // Thực thi và lấy giá trị trả về
            ...
            {
                MessageBox.Show("Lỗi khi tạo mã NCC mới: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
